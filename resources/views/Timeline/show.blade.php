        ...
        @else
            ...
            <div class="entry">
                ...
                <div class="body">
                    ...
                    <p class="hide-in-sm">Membros:</p>
                    @foreach ($trip->tripMembers as $member)
                    <img class="img-fluid foto-perfil hide-in-sm" src="@if($member->userPhoto == 'nophoto') {{asset('./img/icone_user.svg')}} @else {{asset("storage/userPhotos/$member->userPhoto")}} @endif" alt="Foto de {{$trip->userName}}">
                    @endforeach
                    <p>Gasto por membro:</p>
                    <p>R$ {{number_format($trip->foreseen_budget, 0)}}</p>
                </div>
            </div>
            @endforeach
        @endif
