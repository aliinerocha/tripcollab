...
<div class="containerDesktop">
    ...
    <section class="bg-light mt-2 mb-2 pb-1">
        ...
        <form action="{{route('topic.update', ['id' => $topicForm->id])}}" method="POST" class="col-12">
        @csrf 
        @method("PUT")
            <div class="form-group mt-4" >
                <label>Nome do Tópico</label>
                <input class="form-control @error('name') is-invalid @enderror" type="text" name="name" value="{{$topicForm->name}}">
                @error('name')
            ...
            <div class="form-group mt-4">
                ...
                <input textarea class="form-control @error('description') is-invalid @enderror"  type="text" name="description" value="{{$topicForm->description}}">
                @error('description')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
            ...
            <div class="row d-flex justify-content-end m-0">
                <div class="form-group d-flex justify-content-end">
                    <a href="{{route('topic.show',['id' => $topicForm->id] )}}" class="btn botao_atencao mr-2">Cancelar</a>
                    <button type="submit" href="{{route('topic.show',['id' => $topicForm->id] )}}" class="btn botao btn-primary float-right border-0 mr-2">Salvar</button>
                </div>
        </form>
            <form action="{{route('topic.destroy',['id' => $topicForm->id])}}" method="POST">
                @csrf
                @method("DELETE")
    ...
    <!-- Fim Formulário de Edição dos Tópicos -->      
